...
@export_group("Spindash Settings")
@export var spin_dash_min: float = 550
@export var spin_dash_max: float = 1850
@export var spin_dash_acc: float = 75
@export var peel_out_min: float = 600
@export var peel_out_max: float = 2200
@export var peel_out_acc: float = 150
...
@export var topspeed := 1200.0

@export var jump_velocity = 400.0

@export var life := 100

@export var bounce_fall_velocity: float = 1500

@export var bounce_velocity: float = 500
...
func take_damage():
	...
	if Global.Rings > 0:
		Global.Rings -= int(Global.Rings * 0.7)  # Keep it an integer
...
func _ready() -> void:
	presets = System.load_all_presets(preset_path, presets)
...
# Movement
	...
	if direction and not control_lock: # If holding left or right, and not slipping down a slope...
		...
		if is_on_floor() and abs(slopefactor) < 0.25 and !spindash_released: # If you're on flat, or near-flat ground...
			...
			motion.x = move_toward(motion.x, 0.0, acc*2.5)
		else:
...
func test_animate():
	...
	var peel_out_finished: bool = false
	if !facing_direction_lock:
		...
		if motion.x > 0:
			...
			facing_direction = last_facing_direction
	if (!is_on_floor() and !jumping and !bounced and !bouncing_state and !Global.CanRail and !Global.boost_ring and !Global.IsSpringJumping and !spindash_released and !peel_out_released and !is_dropdash_charging) or falling == true:
		if animation.current_animation != "Fall_Start" and animation.current_animation != "Falling":
			animation.play("Fall_Start")
			animation.queue("Falling")
		if motion.y <= 0.0:
			falling = false
	if jumping and jumpAnimStart and !started_jump_anim and !is_dropdash_charging and !is_spindashing_charging and !spindash_released and !Global.CanRail and !bouncing_state and falling == false:
		if motion.y <= 0.0:
			falling = false
			animation.play("Jump")
			...
			falling = true
	if jumping and !jumpAnimStart and !is_dropdash_charging and !Global.CanRail and !bouncing_state and falling == false:
		if motion.y <= 0.0:
			falling = false
	...
	if Global.boost_ring and !is_on_floor():
		...
		jumping = true
#-------------------------------------------SPINDASH ANIM-----------------------------------------
	if is_dropdash_charging or is_spindashing_charging or is_peel_out_charging:
		jumping = false
		animation.play("Spindash_Charge")
		sprite.offset.y = 7.5
		animation.speed_scale = lerp(animation.speed_scale, 4.0, 0.1)
	elif spindash_released or peel_out_released:
		animation.play("Spindash_Released")
		sprite.offset.y = 7.5
	else:
...
#-------------------------------------WALKING, RUNNING, IDLE ANIMS--------------------------------
	if (is_on_floor() or $Collision/Raycast.is_colliding()) and !bouncing_state and !bounced and !spindash_released and !is_peel_out_charging and !peel_out_released and !is_spindashing_charging:
		...
		if ((directionX >= 0 and motion.x >= 50) or (directionX <= 0 and motion.x <= -50)):
				if abs(motion.x) > 50 and abs(motion.x) < 650 and !got_down:
					animation.play("Walk")
					animation.speed_scale = 1 + (abs(motion.x) / 350)
				elif abs(motion.x) >= 650:
					animation.play("Run")
					animation.speed_scale = 1 + (abs(motion.x) / 350)
...
func _wall_movment(delta):
	...
	var perpendicular_velocity = Vector2.DOWN.rotated(rotation) * speed * 23
	motion = velocity
	move_and_slide()
	_wall_animate()
	if Input.is_action_just_pressed("jump"):

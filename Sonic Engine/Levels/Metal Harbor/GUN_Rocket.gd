...
func _process(delta: float) -> void:
	...
	if player_entered:
		...
		player.position = position + Vector2(-100,+7)
		player.control_lock = true
		player.motion = Vector2(0.0,0.0)
	...
	if end_cutscene:
		player.self_modulate = Color(0.794, 0.794, 0.794)
...
func _on_homing_attack_range_body_entered(body: Node2D) -> void:
	EnemyAim.show()
	Global.CanHomeAttack = true

func _on_homing_attack_range_body_exited(body: Node2D) -> void:
	EnemyAim.hide()
	Global.CanHomeAttack = false
